...
A reference field may declare `lookup_filter:`, one boolean expression over the target entity naming which rows are eligible to be chosen -- "only active products on new invoice lines". Every picker narrows by it (the terminal's select and lookup editors, the browser's lookup dialog, remote mode -- each names the reference edge on its query and the server resolves the rule from its own model), and the commit refuses a newly chosen row the criterion excludes, whichever surface chose it. A stored reference that arrives unchanged is never re-checked, so history referencing since-retired rows stays editable; the criterion gates the choosing moment, not the past. The terminal's select editor also keeps the stored row offerable when it is not among the options, which closes the recorded screen crash for stored keys outside the 500-row window.
