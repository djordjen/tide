...
A column's filter popover speaks the mode its field type earns: dates a
From/To pair (either side open-ended), numbers Min/Max, text a
Values-or-Contains toggle, and the enumerable kinds their distinct-value
checklist — one active mode per column, Apply commits exactly one kind,
and emptying everything releases the column. Ranges and contains travel
as the same query operators named filters already use; a saved view
stores them as `conditions` beside its membership map and relights the
bounds on restore. `datetime` columns deliberately keep the checklist
until aware-range semantics are settled.

The web lands on **Home** — assembled, not declared. The saved views the
principal keeps are tiles under *My views*, each wearing its live numbers
(the browse's declared summaries, guaranteed a count, asked of the
ordinary query door under the saved components); the navigation becomes
*Workspaces* tiles with the unfiltered numbers; the capability-filtered
summary reports are shortcuts into the same preview the browse offers. A
saved-view tile opens its browse with the whole state relit. Home travels
as `_tide.home` in the `view` parameter beside administration, keeps the
clean URL as the default landing, and is where an unknown view in a
shared link now falls back. An older server without the catalogue route
simply shows no *My views* section.

The browse toolbar also carries **Export**, where the principal holds
`tide.records.export`: one format renders a button and more than one a menu,
and the control offers exactly what the manifest offered -- which already
filtered by capability and by whether the server has a writer for the format.

What it sends is the conditions on screen, not the rows the browser is
holding: the grid is virtualised and fetches as it scrolls, so what is loaded
is a page rather than the table. The server walks the whole filtered set and
answers with the file. A capped file is announced before the reader trusts it,
naming both the rows they got and the rows the query admits.

The YAML column order remains the shared application default, and an export
uses it rather than a reader's arrangement, so the file is reproducible from
the query alone. Ordinary browser reordering and resizing are personal
preferences and never edit application metadata. New and removed fields reconcile by stable field name. Best Fit
